...
# Test if login works
@test "$T Test login via ldap and keycloak saml" {
    run xvfb-run java -jar /SeleniumTester.jar -h $EE_HOSTNAME -l demouser -p demouser -rr -c /usr/bin/chromedriver -od /host_dir/ 2>/dev/null
    echo "output = ${output}"
}