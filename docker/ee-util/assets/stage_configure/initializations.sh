...
if [ $ENABLE_GUACAMOLE == true ]; then
    ...
    timeout 5 /usql --set SHOW_HOST_INFORMATION=false -C mysql://${RDBMS_GUACAMOLE_USERNAME}:${RDBMS_GUACAMOLE_PASSWORD}@${RDBMS_HOST}:${RDBMS_PORT}/${RDBMS_GUACAMOLE_DATABASE} -t -c "$MYSQL_STRING" -o guac_exists_result.csv
    if grep -q "0" guac_exists_result.csv; then
