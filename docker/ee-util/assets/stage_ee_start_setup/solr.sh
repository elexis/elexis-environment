...
# Configure ELEXIS-SERVER User 
# https://solr.apache.org/guide/8_11/basic-authentication-plugin.html#add-a-user-or-edit-a-password
echo "$T Assert myelexis-server user ..." 
curl -s --user SolrAdmin:${ADMIN_PASSWORD} $SOLR_BASEURL/solr/admin/authentication -H 'Content-type:application/json' -d '{"set-user": { "basic": {"myelexis-server":"'$X_EE_SOLR_ELEXIS_SERVER_PASSWORD'"}}}'

# https://solr.apache.org/guide/8_11/rule-based-authorization-plugin.html#manage-permissions
echo "$T Assert myelexis-server has indexer role ..." 
curl -s --user SolrAdmin:${ADMIN_PASSWORD} $SOLR_BASEURL/solr/admin/authorization -H 'Content-type:application/json' -d '{"set-user-role": {"basic": {"myelexis-server": ["indexer", "user"] }}}'
