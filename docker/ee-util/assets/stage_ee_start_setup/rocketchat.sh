...
curl -s -k $CURL_AUTH -H "Content-type: application/json" $RC_BASEURL/api/v1/channels.create -d '{ "name": "elexis-server", "description": "Elexis-Server status messages" }'

echo -e "\n$T Assert bot user for elexis-user..."
curl -s -k $CURL_AUTH -H "Content-type: application/json" $RC_BASEURL/api/v1/users.create --data-binary @rocketchat/cr_es_user.json

echo -e "\n$T Assert bot avatar for elexis-server ..."
curl -s -k $CURL_AUTH -F "image=@rocketchat/elexis-server.png" -F "username=elexis-server" $RC_BASEURL/api/v1/users.setAvatar 

echo -e "\n$T Assert webhook integration for elexis-server..."
EX_INTEGRATIONS=$(curl -s -k $CURL_AUTH -H "Content-type: application/json" $RC_BASEURL/api/v1/integrations.list)
EXISTING=$(echo $EX_INTEGRATIONS | jq '.integrations[] | select(.name=="elexis-server-messages")')
if [ -z "$EXISTING" ]
then
    echo "Creating webhook ..."
    EXISTING=$(curl -s -k $CURL_AUTH -H "Content-type: application/json" $RC_BASEURL/api/v1/integrations.create --data-binary @rocketchat/cr_es_inc_webhook.json)
fi
echo $EXISTING
...
curl -s -k $CURL_AUTH $RC_BASEURL/api/v1/logout