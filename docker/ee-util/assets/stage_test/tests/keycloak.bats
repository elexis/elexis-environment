...
@test "$T Trigger full user sync from ldap to keycloak (ElexisEnvironmentRealm)" {
    TOKEN=$(curl -s -k -d "client_id=admin-cli" -d "username=KeycloakAdmin" -d "password=$ADMIN_PASSWORD" -d "grant_type=password" https://$EE_HOSTNAME/keycloak/auth/realms/master/protocol/openid-connect/token |jq -r .access_token)
